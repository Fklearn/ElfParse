/*
 * Copyright (c) 2001, 2004, Oracle and/or its affiliates. All rights reserved. DO NOT ALTER OR REMOVE COPYRIGHT NOTICES
 * OR THIS FILE HEADER.
 *
...
public class ELFFileParser {
    ...
    class ELFFileImpl implements ELFFile {
        ...
        class ELFHeaderImpl implements ELFHeader {
            /**
             * Marks the file as an object file and provide machine-independent data so the contents may be decoded and
             * interpreted.
             */
            ...
            /**
             * Virtual address to which the system first transfers control. If there is no entry point for the file the
             * value is 0.
             */
            ...
            /**
             * Size of one entry in the file's program header table in bytes. All entries are the same size.
             */
            ...
            /**
             * Index into the section header table associated with the section name string table. SH_UNDEF if there is
            ...
            /**
             * Reads the ELF header and sets up the section and program headers in memoized arrays.
             */
            ELFHeaderImpl() throws ELFException {
                ...
                for (int i = 0; i < num_sh; i++) {
                    final long sectionHeaderOffset = (long)(sh_offset + (i * sh_entry_size));
            ...
            /**
             * Returns the section header at the specified index. The section header at index 0 is defined as being a
             * undefined section.
             */
            ...
            /**
             * The ELFHashTable does not currently work. This method will always return null.
             */
        ...
        class ELFSectionHeaderImpl implements ELFSectionHeader {
            ...
            private int flags; // Elf32_Word
            /**
             * If the section will be in the memory image of a process this will be the address at which the first byte
                ...
                switch (type) {
                    ...
                    case ELFSectionHeader.TYPE_STRTBL:
                        ...
                        stringTable = new MemoizedObject() {
                            public Object computeValue() {
                                return new ELFStringTableImpl(strTableOffset, strTableSize);
                            }
                        };
                        break;
                    ...
                    case ELFSectionHeader.TYPE_HASH:
                        ...
                        hashTable = new MemoizedObject() {
                            public Object computeValue() {
                                return new ELFHashTableImpl(hashTableOffset, hashTableSize);
                            }
                        };
                        break;
            ...
            /**
             * Returns the ELFSymbol at the specified index. Index 0 is reserved for the undefined ELF symbol.
             */
        ...
        class ELFSymbolImpl implements ELFSymbol {
            /**
             * Index into the symbol string table that holds the character representation of the symbols. 0 means the
             * symbol has no character name.
             */
            private int name_ndx; // Elf32_Word
            /**
             * Value of the associated symbol. This may be an address or an absolute value.
             */
            ...
            /**
             * Index to the associated section header. This value will need to be read as an unsigned short if we
             * compare it to ELFSectionHeader.NDX_LORESERVE and ELFSectionHeader.NDX_HIRESERVE.
             */
        ...
        class ELFStringTableImpl implements ELFStringTable {
            ...
            ELFStringTableImpl(long offset, int length) throws ELFException {
                ...
                if (bytesRead != length) {
                    throw new ELFException("Error reading string table (read " + bytesRead + "bytes, expected to "
        ...
        class ELFHashTableImpl implements ELFHashTable {
            ...
            ELFHashTableImpl(long offset, int length) throws ELFException {
                ...
                if (length != actual) {
                    throw new ELFException("Error reading string table (read " + actual + "bytes, expected to "
                        + "read " + length + "bytes).");
                }
            }

            /**
             * This method doesn't work every time and is unreliable. Use ELFSection.getELFSymbol(String) to retrieve
             * symbols by name. NOTE: since this method is currently broken it will always return null.
             */
        ...
        int unsignedByte(short arg) {
            ...
            if (arg >= 0) {
                ...
                val = (int)(((int)unsignedByte((byte)(arg >>> 8)) << 8) | ((byte)arg));
            }
            return val;
        }
        ...
        long unsignedByte(int arg) {
            ...
            if (arg >= 0) {
                ...
                val = (long)(((long)unsignedByte((short)(arg >>> 16)) << 16) | ((short)arg));
            }
            return val;
        }

        /**
         * Unsigned byte utility functions used for converting from big-endian (MSB) to little-endian (LSB).
         */
        int unsignedByteSwap(short arg) {
            return (int)(((int)unsignedByte((byte)arg)) << 8) | ((int)unsignedByte((byte)(arg >>> 8)));
        }

        long unsignedByteSwap(int arg) {
            return (long)(((long)unsignedByteSwap((short)arg)) << 16) | ((long)unsignedByteSwap((short)(arg >>> 16)));
        }
    }
    ...
    public static void main(String args[]) {
        ...
        // }
        // parserElf(args[0]);
        parserElf("libgetstr.so");
    }

    private static void parserElf(String fn) {

        System.out.println("ELF File: " + fn);
        // Parse the file.
        ELFFile elfFile = ELFFileParser.getParser().parse(fn);

        ELFHeader elfHeader = elfFile.getHeader();

        System.out.println("ELF object size: " + ((elfFile.getObjectSize() == 0) ? "Invalid Object Size"
            : (elfFile.getObjectSize() == 1) ? "32-bit" : "64-bit"));
        System.out.println("ELF data encoding: "
            + ((elfFile.getEncoding() == 0) ? "Invalid Data Encoding" : (elfFile.getEncoding() == 1) ? "LSB" : "MSB"));
